...
use disks::{app_info, open_fda_settings, list_disks, inspect_image, start_write, cancel_write, verify_image, CancelRegistry};
use db::{
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      match db::open(&app.handle()) {
        Ok(conn) => { app.manage(Db(Mutex::new(conn))); }
        Err(e) => { eprintln!("db::open failed, continuing without persistence: {e}"); }
      }
