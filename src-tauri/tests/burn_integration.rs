...
//!
//! Each test wires together a custom in-memory streaming source and the
//! on-disk `PlainFileDeviceIo` (the "target"), runs `burn`, then re-reads
//! the target file and runs `verify_hash_only` or `verify`.
//!
...
/// size alongside the cursor so the pipeline call sites can pass it.
struct TestSource {
    data: Cursor<Vec<u8>>,
    total: u64,
}
...
fn burn_then_verify_hash_only_matches() {
    ...
    let mut reader = TestSource::new(data.clone());
...
fn burn_then_verify_byte_compare_matches() {
    ...
    let mut source_reader = TestSource::new(data.clone());
    let total = source_reader.total;
...
fn cancel_mid_burn_returns_cancelled_error() {
    ...
    let mut reader = TestSource::new(data);
    let total = reader.total;
...
fn empty_image_burns_clean() {
    ...
    let mut reader = TestSource::new(Vec::new());
    let total = reader.total;
